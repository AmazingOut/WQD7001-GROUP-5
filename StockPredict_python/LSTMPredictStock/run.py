...
def format_predictions(predictions):    # 给预测数据添加对应日期
    ...
    for stock_code in companies.keys():
        with open(stock_code + "_pre.csv", 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerows(date_predict)
...
def plot_results(date_predict, true_data):  # predicted_data与true_data：同长度一维数组
    fig = plt.figure(facecolor='white')
    ax = fig.add_subplot(111)
    ax.plot(true_data, label='True Data')
    plt.plot(date_predict, label='Prediction')
    plt.legend()
    plt.show()
